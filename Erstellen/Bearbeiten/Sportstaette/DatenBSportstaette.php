...
<h1>Sportstätte bearbeiten</h1></br></br>
</div>
		...
		else
		{
			...
			else
			{
				...
				if ($found == 1)
				{
					...
					$sql = 'UPDATE sportstaette SET Name='.$Name.', Telefonnummer='.$TelNum.', MailAdresse='.$EMail.', Internetseite='.$Internet.', Strasse='.$Str.', Hausnummer='.$Hausnummer.', Postleitzahl='.$Plz.', Ort='.$Ort.', Land='.$Land.', Koordinaten='.$Koords.' WHERE ID='.$ID.';';
					$result = $conn->query($sql);
					...
					if ($result === TRUE)
					{
						...
						header('Location: AttBSportstaette.php');
					}
					else
					{
...
<div>
<form action ="DatenBSportstaette.php" method="Post">
