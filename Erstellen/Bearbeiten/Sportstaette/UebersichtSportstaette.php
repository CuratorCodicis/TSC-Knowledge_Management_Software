...
<h1 style="text-align:center;">Sportstätte bearbeiten</h1></br></br>
</div></br>
...
if($_SERVER["REQUEST_METHOD"]=="POST" && !isset($_POST["ID"])) {
	...
	{
		$Anfragetext = $_POST["Str"];
		if(isset($_POST["Hausnummer"])) {$Anfragetext = $Anfragetext.' '.$_POST["Hausnummer"];}
		if(isset($_POST["Plz"])) {$Anfragetext = $Anfragetext.' '.$_POST["Plz"];}
		$Anfragetext = $Anfragetext.' '.$_POST["Ort"];
				...
				if ($found == 1)
				{
					...
					else 
					{
						...
						while ($row=$result->fetch_assoc()) {
							...
							if(isset($_POST[$ConvertedName.'x'])&& $_POST[$ConvertedName.'x']!=NULL) {
								$sql2 = 'INSERT INTO eignungsass (SSID, SAName) 
										VALUES ('.$ID.', "'.$row["Name"].'");';
						...
						$sql='SELECT AName FROM attributefuersportstaette WHERE Typ="bool";';
						$result = $conn->query($sql);
						...
						while ($row=$result->fetch_assoc()) {
							...
							if(isset($_POST[$ConvertedAName])&& $_POST[$ConvertedAName]!=NULL) {
								$sql2 = 'INSERT INTO ssbesitzt_bool (SSID, AName, Wert) 
						...
						$sql='SELECT AName FROM attributefuersportstaette WHERE Typ="int";';
						$result = $conn->query($sql);
						...
						while ($row=$result->fetch_assoc()) {
							$ConvertedAName = str_replace(" ","_",$row["AName"]);
							if(isset($_POST[$ConvertedAName]) && $_POST[$ConvertedAName]!=NULL) {
								$sql2 = 'INSERT INTO ssbesitzt_int (SSID, AName, Wert) 
										VALUES ('.$ID.', "'.$row["AName"].'", '.$_POST[$ConvertedAName].');';
								$result2 = $conn->query($sql2);
						...
						$sql='SELECT AName FROM attributefuersportstaette WHERE Typ="char" OR Typ="charc";';
						$result = $conn->query($sql);
						...
						while ($row=$result->fetch_assoc()) {
							...
							for($k=0;$k<count($charArray);$k++) {
								$sql2 = 'INSERT INTO ssbesitzt_char (SSID, AName, Wert) 
...
echo '
<div>
<form action ="UebersichtSportstaette.php" method="Post">
...
$sql = "SELECT * FROM sportstaette where ID=".$ID.";";
$result = $conn -> query($sql);
...
if($result === false) {
	...
	while($row=$result->fetch_assoc()){
		...
		if($errorPflicht == true){echo 'style="border: 2px solid red;"';}
		echo '></td></tr>';
...
$sql = "SELECT * FROM eignungsass WHERE SSID=".$ID." ORDER BY SAName;";
$result = $conn -> query($sql);
...
if($result === false) {
	...
	while($row=$result->fetch_assoc()){
		$ConvertedName = str_replace(" ","_",$row["SAName"]);
		if($_SERVER["REQUEST_METHOD"]=="POST" && !isset($_POST["ID"]) && !$updatedDB) {
			if(isset($_POST[$ConvertedName.'x'])) {
				echo '<tr><th>'.$row["SAName"].'</th><td><input type="checkbox" value=1 name="'.$row["SAName"].'x'.'" checked></td></tr><tr><th></br></th><td></br></td></tr>';
			} else {
				echo '<tr><th>'.$row["SAName"].'</th><td><input type="checkbox" value=1 name="'.$row["SAName"].'x'.'"> </td></tr><tr><th></br></th><td></br></td></tr>';	
			}
		} else {
			echo '<tr><th>'.$row["SAName"].'</th><td><input type="checkbox" value=1 name="'.$row["SAName"].'x'.'" checked> </td></tr><tr><th></br></th><td></br></td></tr>';	
		}
...
//bool Attribute
$sql = "SELECT * FROM ssbesitzt_bool WHERE SSID=".$ID." ORDER BY AName;";
$result = $conn -> query($sql);
...
//char Attribute
$sql = "SELECT * FROM ssbesitzt_char NATURAL JOIN attributefuersportstaette WHERE SSID=".$ID." ORDER BY AName;";
$result = $conn -> query($sql);
...
if($result === false) {
	...
	while($row=$result->fetch_assoc()){
		...
		if($row["Typ"] == "char") {
			echo '<tr><th>'.$row["AName"].'</th><td><input type="text" name="'.$row["AName"].$m.' value="';
			if($_SERVER["REQUEST_METHOD"]=="POST" && !isset($_POST["ID"]) && !$updatedDB) {echo $_POST[$ConvertedAName.$m].'"';}
			else {echo $row["Wert"].'"';}
			echo '></td></tr>';
			$m++;
		}
		else if ($row["Typ"] == "charc") {
			$sql2 = "SELECT Wert FROM attributefuersportstaetteauswahlwerte WHERE AName='".$row["AName"]."' ORDER BY Wert;";
			$result2 = $conn->query($sql2);
			...
			if($_SERVER["REQUEST_METHOD"]=="POST" && !isset($_POST["ID"]) && !$updatedDB) {if ($_POST[$ConvertedAName.$m]=="Keine Angabe") {echo ' selected';}}
			else {if ($row["Wert"]=="Keine Angabe") {echo ' selected';}}
			echo '>Keine Angabe</option>';
			
			while($row2=$result2->fetch_assoc()){
				echo '<option value="'.$row2["Wert"].'"';
				if($_SERVER["REQUEST_METHOD"]=="POST" && !isset($_POST["ID"]) && !$updatedDB) {if($_POST[$ConvertedAName.$m]==$row2["Wert"]) {echo ' selected';}}
				else {if($row["Wert"]==$row2["Wert"]) {echo ' selected';}}
...
if($result === false) {
	...
	while($row=$result->fetch_assoc()){
		...
		if($_SERVER["REQUEST_METHOD"]=="POST" && !isset($_POST["ID"]) && !$updatedDB) {
			...
			echo '<tr><th>'.$row["AName"].'</th><td><input type="number" id="number" name="'.$row["AName"].'" value='.$_POST[$ConvertedAName].'></td></tr>';
		} else {
			echo '<tr style="height: 10px"><td></td></tr>';
...
<td><a href="DatenBSportstaette.php">Alle Daten bearbeiten</a></td>
<td><a href="LoeschenSportstaette.php" id="deleteB">Sportstätte löschen</a></td>
